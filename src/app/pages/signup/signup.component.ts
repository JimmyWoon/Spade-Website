...
export class SignupComponent  {
  ...
  updateCountryButton(selectedText: string, index:number): void {
    this.formGroup.controls["country"].setValue(selectedText);
    this.states = this.state_list[index].split("|"); 
    this.isButtonDisabled = false; // Enable the button

  }

  updateStateButton(state:string){
  ...
  submit() {
    ...
    const dobControl = this.formGroup.get('dob');
    const first_nameControl = this.formGroup.get('first_name');
    const last_nameControl = this.formGroup.get('last_name');

    
     if (this.formGroup.valid && stateControl?.value !== "State" && countryControl?.value !== "Country" ){
          ...
          if (passControl?.value === pass_confirmControl?.value){
            ...
            this.isEmailRegistered(emailControl?.value).then((isRegistered) => {
              if (isRegistered) {
                ...
                this.cookieService.set('newuser-first-name', first_nameControl?.value, 0.5);  
                this.cookieService.set('newuser-last-name', last_nameControl?.value, 0.5);  
                this.cookieService.set('newuser-dob', dobControl?.value, 0.5);  
                this.cookieService.set('newuser-state', stateControl?.value, 0.5);  
                this.cookieService.set('newuser-country', countryControl?.value, 0.5);  
